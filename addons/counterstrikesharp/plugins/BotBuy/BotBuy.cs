...
//----------------------------------------------------------------------------------------------
        ...
        params (string Weapon, int Price)[] values)
    {
        ...
        {
            yield return new PrimaryPurchaseCandidate(
                value.Weapon, value.Price, PurchaseArmor.Full);
            yield return new PrimaryPurchaseCandidate(
                value.Weapon, value.Price, PurchaseArmor.Half);
            if (value.Weapon == "weapon_awp" || !RiflePurchasePolicy.IsRifle(value.Weapon))
            {
                yield return new PrimaryPurchaseCandidate(
                    value.Weapon, value.Price, PurchaseArmor.None);
            }
        }
    }
    ...
    private static bool IsPrimaryWeapon(string weapon)
        => RiflePurchasePolicy.IsPrimaryWeapon(weapon);
